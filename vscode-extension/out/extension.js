...
function activate(context) {
    ...
    function mapSeverity(vowSeverity) {
    ...
    // Run Vow check
    async function runVowCheck(target) {
        const config = vscode.workspace.getConfiguration('vow');
        const executablePath = config.get('executablePath', 'vow');
    ...
    function updateDiagnostics(result) {
        ...
        for (const issue of result.issues) {
            ...
            const range = new vscode.Range(new vscode.Position(line, column), new vscode.Position(line, column + 10) // Approximate range
            );
    ...
    const checkFileCommand = vscode.commands.registerCommand('vow.checkFile', async () => {
        ...
        if (result) {
            ...
            vscode.window.showInformationMessage(totalIssues === 0
    ...
    const showReportCommand = vscode.commands.registerCommand('vow.showReport', async () => {
        ...
        const panel = vscode.window.createWebviewPanel('vowReport', 'Vow Report', vscode.ViewColumn.Two, { enableScripts: true });
        ...
        return `
            ...
            <body>
                ...
                <div class="summary">
                    ...
                    ${Object.entries(severityCounts).map(([severity, count]) => `<p>${severity}: ${count}</p>`).join('')}
                </div>
    ...
    context.subscriptions.push(diagnosticCollection, statusBarItem, checkFileCommand, checkWorkspaceCommand, showReportCommand, onSaveDisposable);
}
