...
pub fn hooks_install_pre_push() -> Result<(), Box<dyn std::error::Error>> {
    // Check if we're in a git repository
    let git_dir = PathBuf::from(".git");
    if !git_dir.exists() {
        return Err("Not in a git repository. Run 'git init' first.".into());
    ...
    if hook_path.exists() {
        // Back up existing hook
        let backup_path = hooks_dir.join("pre-push.vow-backup");
        fs::copy(&hook_path, &backup_path)?;
        println!("⚠️  Existing pre-push hook backed up to pre-push.vow-backup");
        
        // Read existing hook content
        let existing_content = fs::read_to_string(&hook_path)?;
        if existing_content.contains("# Vow pre-push hook") {
            println!("✓ Vow pre-push hook already installed");
            return Ok(());
        }
        
        // Chain with existing hook
        let hook_content = generate_pre_push_hook_script_with_chain(&existing_content)?;
        fs::write(&hook_path, hook_content)?;
    } else {
        // Create new hook
        let hook_content = generate_pre_push_hook_script()?;
        fs::write(&hook_path, hook_content)?;
    }
    ...
    println!("   The hook will run 'vow check --diff' on the push range before each push");
    println!("   Use 'git push --no-verify' to bypass the hook");
    
    Ok(())
}

/// Uninstall pre-push git hook
pub fn hooks_uninstall_pre_push() -> Result<(), Box<dyn std::error::Error>> {
    let git_dir = PathBuf::from(".git");
    if !git_dir.exists() {
        return Err("Not in a git repository".into());
    }
    ...
    if hook_content.contains("# Vow pre-push hook") {
        // This is our hook or contains our hook
        if backup_path.exists() {
            // Restore backup
            fs::copy(&backup_path, &hook_path)?;
            fs::remove_file(&backup_path)?;
            println!("✅ Vow pre-push hook removed, original hook restored");
        } else {
            // Remove the hook entirely
            fs::remove_file(&hook_path)?;
            println!("✅ Vow pre-push hook removed");
        }
    } else {
        println!("⚠️  Pre-push hook exists but doesn't appear to be installed by Vow");
        return Err("Cannot safely remove hook not installed by Vow".into());
    }
...
fn generate_pre_push_hook_script() -> Result<String, Box<dyn std::error::Error>> {
    // Try to find the best vow command to use
    let vow_cmd = find_vow_command();
    
    let script = format!(r#"#!/bin/sh
# Vow pre-push hook
# Auto-generated by vow hooks install --pre-push

# Read stdin for remote information
while IFS=' ' read local_ref local_sha remote_ref remote_sha; do
    ...
    # If remote_sha is all zeros, this is a new branch - compare against HEAD~10 or initial commit
    if [ "$remote_sha" = "0000000000000000000000000000000000000000" ]; then
        # New branch - get the merge base with main/master or use HEAD~10
        main_branch=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@' || echo "main")
        if git rev-parse --verify "origin/$main_branch" >/dev/null 2>&1; then
            remote_sha=$(git merge-base "origin/$main_branch" "$local_sha" 2>/dev/null || echo "HEAD~10")
        else
            # Fallback to first commit or HEAD~10
            remote_sha=$(git rev-list --max-parents=0 HEAD 2>/dev/null | tail -1 || echo "HEAD~10")
        fi
    fi
    
    # Check if we have any commits to analyze
    if ! git rev-parse --verify "$remote_sha" >/dev/null 2>&1; then
        echo "⚠️  Cannot find base commit $remote_sha, skipping Vow check"
        continue
    fi
    
    # Create diff range for vow
    diff_range="$remote_sha..$local_sha"
    
    # Run vow check with diff mode
    {vow_cmd} check . --diff "$diff_range" --quiet --format terminal
    ...
    if [ $exit_code -ne 0 ]; then
        echo ""
        echo "❌ Vow found issues in files being pushed. Push blocked."
        echo "   Fix the issues above, or use 'git push --no-verify' to bypass."
        echo "   Checked commits: $diff_range"
        exit 1
    fi
done
...
/// Generate pre-push hook script that chains with existing hook
fn generate_pre_push_hook_script_with_chain(existing_content: &str) -> Result<String, Box<dyn std::error::Error>> {
    let vow_cmd = find_vow_command();
    
    let script = format!(r#"#!/bin/sh
# Vow pre-push hook (chained with existing hook)
# Auto-generated by vow hooks install --pre-push

# === VOW CHECK START ===
# Read stdin for remote information (save a copy for the original hook)
input_data=$(cat)

echo "$input_data" | while IFS=' ' read local_ref local_sha remote_ref remote_sha; do
    # Skip deleted branches
    if [ "$local_sha" = "0000000000000000000000000000000000000000" ]; then
        continue
    fi
...
# If vow check failed, exit early
if [ $? -ne 0 ]; then
    exit 1
fi
# === VOW CHECK END ===
...
# === ORIGINAL HOOK START ===
# Pass the input data to the original hook
echo "$input_data" | {{
{existing_hook}
}}
# === ORIGINAL HOOK END ===
...
"#;
    ...
    #[test]
    fn test_generate_pre_push_hook_script() {
        let script = generate_pre_push_hook_script().unwrap();
        
        // Should contain our hook marker
        assert!(script.contains("# Vow pre-push hook"));
        
        // Should read stdin for remote information
        assert!(script.contains("while IFS=' ' read local_ref local_sha remote_ref remote_sha"));
        
        // Should contain diff flag
        assert!(script.contains("--diff"));
        
        // Should handle deleted branches
        assert!(script.contains("0000000000000000000000000000000000000000"));
        
        // Should handle new branches
        assert!(script.contains("main_branch=$(git symbolic-ref"));
    ...
    #[test]
    fn test_generate_pre_push_hook_script_with_chain() {
        let existing_script = "#!/bin/sh\n# Existing hook\necho 'existing hook logic'";
        let script = generate_pre_push_hook_script_with_chain(existing_script).unwrap();
        
        // Should contain our hook marker
        assert!(script.contains("# Vow pre-push hook (chained with existing hook)"));
        
        // Should contain VOW CHECK sections
        assert!(script.contains("=== VOW CHECK START ==="));
        assert!(script.contains("=== VOW CHECK END ==="));
        
        // Should contain original hook sections
        assert!(script.contains("=== ORIGINAL HOOK START ==="));
        assert!(script.contains("=== ORIGINAL HOOK END ==="));
        
        // Should include the existing hook content
        assert!(script.contains("echo 'existing hook logic'"));
        
        // Should save input data for both hooks
        assert!(script.contains("input_data=$(cat)"));
    }
    ...
    #[test]
    fn test_hooks_install_pre_push_no_git_repo() {
        let temp_dir = TempDir::new().unwrap();
        let original_dir = env::current_dir().unwrap();
        env::set_current_dir(temp_dir.path()).unwrap();
        
        // Should fail when not in a git repo
        let result = hooks_install_pre_push();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Not in a git repository"));
