...
static SECURITY_PATTERNS: Lazy<Vec<SecurityPattern>> = Lazy::new(|| vec![
    ...
    // Additional Rust vulnerability patterns for missing detections
    
    // Issue #16: Command injection in Rust
    SecurityPattern {
        name: "rust_command_format",
        regex: Regex::new(r#"(?i)Command::new\s*\([^)]*\)\s*\.arg\s*\(\s*(?:format!|&format!)\s*\([^)]*[a-zA-Z_][a-zA-Z0-9_]*"#).unwrap(),
        severity: Severity::High,
        message: "Rust command injection - format! macro with user input in command arguments",
    },
    SecurityPattern {
        name: "rust_command_injection_shell",
        regex: Regex::new(r#"(?i)Command::new\s*\(\s*["']sh["']\s*\)\s*\.arg\s*\(\s*["']-c["']\s*\)\s*\.arg\s*\(\s*&[a-zA-Z_][a-zA-Z0-9_]*"#).unwrap(),
        severity: Severity::Critical,
        message: "Rust command injection - shell execution with user input",
    },
    SecurityPattern {
        name: "rust_command_injection_direct",
        regex: Regex::new(r#"(?s)Command::new\s*\(\s*["']sh["']\s*\)[^}]*\.arg\s*\(\s*["']-c["']\s*\)[^}]*\.arg\s*\(\s*&\*[a-zA-Z_]"#).unwrap(),
        severity: Severity::Critical,
        message: "Rust command injection - direct user input to shell command",
    },
    SecurityPattern {
        name: "rust_command_injection_general",
        regex: Regex::new(r#"(?s)Command::new.*?\.arg.*?\.arg\s*\(\s*&\*[a-zA-Z_]"#).unwrap(),
        severity: Severity::High,
        message: "Rust command injection - user input in command arguments",
    },
    ...
    SecurityPattern {
        ...
        regex: Regex::new(r#"(?i)reqwest::(get|post|put|delete|head)\s*\(\s*&[a-zA-Z_][a-zA-Z0-9_]*"#).unwrap(),
        severity: Severity::High,
        message: "Rust SSRF - HTTP request with user-controlled URL",
    },
    SecurityPattern {
        name: "rust_ssrf_client",
        regex: Regex::new(r#"(?i)Client::new\(\)[^;]*\.(get|post|put|delete)\s*\(\s*&[a-zA-Z_][a-zA-Z0-9_]*"#).unwrap(),
        severity: Severity::High,
        message: "Rust SSRF - HTTP client request with user input",
    },
    SecurityPattern {
        name: "rust_ssrf_format_url",
        regex: Regex::new(r#"(?i)reqwest::(get|post|put|delete|head)\s*\(\s*&?format!\s*\([^)]*https?://[^)]*[a-zA-Z_][a-zA-Z0-9_]*"#).unwrap(),
        severity: Severity::Critical,
        message: "Rust SSRF - HTTP request with format! constructed URL containing user input",
    },
    
    // Issue #28-#35: Memory safety issues in unsafe blocks
    SecurityPattern {
        name: "rust_unsafe_use_after_free",
        regex: Regex::new(r"(?s)unsafe\s*\{[^}]*drop\s*\([^}]*Box::from_raw[^}]*\*[a-zA-Z_]").unwrap(),
        severity: Severity::Critical,
        message: "Rust unsafe memory issue - potential use after free with Box::from_raw",
    },
    SecurityPattern {
        name: "rust_unsafe_double_free", 
        regex: Regex::new(r"(?s)unsafe\s*\{[^}]*drop\s*\([^}]*drop\s*\([^}]*Box::from_raw").unwrap(),
        severity: Severity::Critical,
        message: "Rust unsafe memory issue - potential double free with multiple Box::from_raw",
    },
    SecurityPattern {
        name: "rust_unsafe_buffer_overflow",
        regex: Regex::new(r"(?s)unsafe\s*\{[^}]*\*[a-zA-Z_][a-zA-Z0-9_]*\.offset\s*\(\s*[0-9]+\s*\)\s*=").unwrap(),
        severity: Severity::High,
        message: "Rust unsafe memory issue - potential buffer overflow with pointer offset",
    },
    SecurityPattern {
        name: "rust_memory_uninit",
        regex: Regex::new(r"(?s)unsafe\s*\{[^}]*MaybeUninit::uninit\(\)[^}]*assume_init\(\)").unwrap(),
        severity: Severity::High,
        message: "Rust unsafe memory issue - using uninitialized memory with assume_init",
    },
    SecurityPattern {
        name: "rust_unsafe_transmute",
        regex: Regex::new(r"(?s)unsafe\s*\{[^}]*std::mem::transmute").unwrap(),
        severity: Severity::Medium,
        message: "Rust unsafe memory issue - type confusion with transmute",
    },
    SecurityPattern {
        name: "rust_unsafe_general",
        regex: Regex::new(r"(?s)unsafe\s*\{").unwrap(),
        severity: Severity::Low,
        message: "Rust unsafe block detected - verify memory safety",
    },
    
    // Issue #29-#30: Race conditions and concurrency issues
    SecurityPattern {
        name: "rust_global_mutable_static",
        regex: Regex::new(r"(?i)static\s+mut\s+[A-Z_]+.*?:\s*[^=]+=").unwrap(),
        severity: Severity::High,
        message: "Rust global mutable static - potential race condition",
    },
    SecurityPattern {
        name: "rust_race_condition_refcell",
        regex: Regex::new(r"(?s)Arc::new\(std::cell::RefCell::new\([^}]*thread::spawn[^}]*borrow_mut\(\)").unwrap(),
        severity: Severity::Medium,
        message: "Rust race condition - RefCell in Arc across threads causing concurrency issue",
    },
    SecurityPattern {
        name: "rust_race_condition_cell",
        regex: Regex::new(r"(?s)Arc::new\(std::cell::Cell::new\([^}]*val \+ 1").unwrap(),
        severity: Severity::Medium,
        message: "Rust race condition - Cell in Arc with read-modify-write causing concurrency issue",
    },
    SecurityPattern {
        name: "rust_concurrency_unsafe_global",
        regex: Regex::new(r"(?s)unsafe\s*\{[^}]*[A-Z_]+\s*\+=").unwrap(),
        severity: Severity::High,
        message: "Rust concurrency issue - unsafe global access creating potential race condition",
    },
    SecurityPattern {
        name: "rust_concurrency_race_condition",
        regex: Regex::new(r"(?s)Arc::new\(std::cell::(RefCell|Cell)::new\([^}]*thread::spawn").unwrap(),
        severity: Severity::Medium,
        message: "Rust concurrency race condition - shared mutable state across threads",
    },
    
    // Additional CSRF detection patterns
    SecurityPattern {
        name: "rust_csrf_delete_endpoint",
        regex: Regex::new(r"(?i)pub\s+async\s+fn\s+delete_[a-zA-Z_][a-zA-Z0-9_]*\s*\([^)]*web::Path").unwrap(),
        severity: Severity::Medium,
        message: "Rust DELETE endpoint - missing CSRF protection for state-changing operation",
    },
    SecurityPattern {
        name: "rust_csrf_money_transfer",
        regex: Regex::new(r"(?i)pub\s+async\s+fn\s+[a-zA-Z_]*transfer[a-zA-Z_]*\s*\([^)]*web::(Json|Form)").unwrap(),
        severity: Severity::High,
        message: "Rust money transfer endpoint - missing CSRF protection",
    },
    SecurityPattern {
        name: "rust_csrf_password_change",
        regex: Regex::new(r"(?i)pub\s+async\s+fn\s+[a-zA-Z_]*(?:change|update)[a-zA-Z_]*password[a-zA-Z_]*\s*\([^)]*web::(Json|Form)").unwrap(),
        severity: Severity::High,
        message: "Rust password change endpoint - missing CSRF protection",
    },
    
    // Environment variable access pattern
    SecurityPattern {
        name: "rust_env_var_access",
        regex: Regex::new(r#"(?i)std::env::var\s*\(\s*["'][^"']*(?:PASSWORD|SECRET|KEY|TOKEN|API)[^"']*["']\s*\)"#).unwrap(),
        severity: Severity::Low,
    ...
    // Java/Servlet vulnerability patterns (#468-#475)
    ...
    // #469: XSS - unsanitized getParameter() output to response
    SecurityPattern {
    ...
    // #470: Path traversal - user-controlled file path
    SecurityPattern {
    ...
    SecurityPattern {
        name: "java_servlet_path_traversal_variable",
        regex: Regex::new(r#"(?i)(?:String\s+\w+\s*=\s*request\.getParameter\s*\([^)]*\)\s*;[^}]*new\s+(?:File|FileInputStream)\s*\([^)]*\w+)"#).unwrap(),
        severity: Severity::High,
        message: "Path traversal in Java Servlet - user-controlled variable used in file path",
    },
    
    // #471: SSRF - user-controlled URL in URL.openConnection
    SecurityPattern {
    ...
    SecurityPattern {
        name: "java_servlet_ssrf_variable",
        regex: Regex::new(r"(?i)(?:String\s+\w+\s*=\s*request\.getParameter\s*\([^)]*\)\s*;[^}]*new\s+URL\s*\([^)]*\w+[^)]*\)\s*\.\s*openConnection)").unwrap(),
        severity: Severity::High,
        message: "SSRF in Java Servlet - user input used in URL.openConnection()",
    },
    
    // #472: Open redirect - user-controlled URL in sendRedirect
    SecurityPattern {
    ...
    SecurityPattern {
        name: "java_servlet_open_redirect_variable",
        regex: Regex::new(r"(?i)(?:String\s+\w+\s*=\s*request\.getParameter\s*\([^)]*\)\s*;[^}]*(?:response|res)\s*\.\s*sendRedirect\s*\([^)]*\w+)").unwrap(),
        severity: Severity::Medium,
        message: "Open redirect in Java Servlet - user-controlled variable in sendRedirect()",
    },
    
    // #473: Hardcoded secrets in Java
    SecurityPattern {
    ...
    // #474: Insecure deserialization - ObjectInputStream.readObject
    SecurityPattern {
    ...
    // #475: CSRF - POST handlers without CSRF token validation
    SecurityPattern {
...
impl CodeAnalyzer {
    ...
    fn should_apply_pattern(&self, pattern_name: &str, file_type: &FileType, path: &Path) -> bool {
        match pattern_name {
            ...
            // Java/Servlet security patterns
            "java_servlet_sql_injection" | "java_servlet_sql_injection_concat" |
            "java_servlet_xss" | "java_servlet_xss_variable" |
            "java_servlet_path_traversal" | "java_servlet_path_traversal_variable" |
            "java_servlet_ssrf" | "java_servlet_ssrf_variable" |
            "java_servlet_open_redirect" | "java_servlet_open_redirect_variable" |
            "java_hardcoded_password" | "java_hardcoded_credential" |
            "java_insecure_deserialization" | "java_insecure_deserialization_inline" |
            "java_servlet_csrf_dopost" | "java_servlet_csrf_post_handler" => {
                matches!(file_type, FileType::Java)
            },
